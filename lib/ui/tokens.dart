// AutoSub Media Player — Design Tokens (the Dart source of truth).
//
// Realizes docs/DESIGN_SYSTEM.md §3 + docs/design/TOKENS.dart. Wired into the app
// via appTheme() in main.dart. NEVER hardcode a hex/size/duration in a widget —
// add or reuse a token here (DESIGN_SYSTEM §10 governance).
//
// Dark-only for v1 (DESIGN_SYSTEM §2). The semantic layer keeps a future light
// theme a token swap, not a refactor.

import 'package:flutter/material.dart';

// ---------------------------------------------------------------------------
// Color — raw ramp (defines roles; reference the semantic roles in widgets).
// ---------------------------------------------------------------------------
...
abstract final class AppColors {
  ...
  static const neutral700 = Color(0xFF2A313A); // hairline dividers / borders
  ...
  /// Hairline divider/border color (neutral700 @ ~60% α — DESIGN_SYSTEM §3.1).
  static const hairline = Color(0x992A313A);
}

/// A status's (foreground, tint) pair + spoken word + icon — for StatusChip.
class StatusStyle {
  const StatusStyle(this.fg, this.tint, this.spoken, this.icon);
  final Color fg;
  final Color tint;
  final String spoken;
  final IconData icon;
...
/// Map JobState / title-status keys to their style + icon (DESIGN_SYSTEM §3.1).
abstract final class StatusStyles {
  static const ready = StatusStyle(
      AppColors.readyFg, AppColors.readyTint, 'Ready', Icons.check_circle);
  static const running = StatusStyle(
      AppColors.runningFg, AppColors.runningTint, 'Translating', Icons.autorenew);
  static const queued = StatusStyle(
      AppColors.queuedFg, AppColors.queuedTint, 'Queued', Icons.schedule);
  static const paused = StatusStyle(
      AppColors.pausedFg, AppColors.pausedTint, 'Paused', Icons.pause_circle);
  static const failed = StatusStyle(
      AppColors.failedFg, AppColors.failedTint, 'Failed', Icons.error);
  static const attention = StatusStyle(AppColors.attentionFg,
      AppColors.attentionTint, 'Needs attention', Icons.warning_amber);

  /// Resolve the style for an engine/job state wire-string
  /// (queued | running | paused | failed | done). `done`/ready → [ready].
  static StatusStyle forState(String state) {
    switch (state) {
      case 'running':
        return running;
      case 'queued':
...
// ---------------------------------------------------------------------------
// Motion (halve / disable under MediaQuery.disableAnimations — DS §3.6/§6.6)
// ---------------------------------------------------------------------------
...
// When the .ttf assets aren't bundled, Flutter falls back to the platform font
// (San Francisco on macOS) which still resolves Hebrew — the metrics differ but
// layout holds. Bundle the OFL fonts via pubspec `fonts:` to match the spec.
// ---------------------------------------------------------------------------
  ...
  return ThemeData(
    ...
    inputDecorationTheme: InputDecorationTheme(
      ...
      border: const OutlineInputBorder(
          borderRadius: AppRadius.borderSm, borderSide: BorderSide.none),
